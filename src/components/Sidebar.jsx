...
const Sidebar = () => {
    ...
    return (
        <div className='w-16 md:w-56 flex flex-col fixed left-0 top-0 bottom-0 z-10 pt-8 px-4 bg-[#0f172a] border-r border-slate-700 shadow-lg shadow-black/20'>
            <div className='mb-8'>
                <img src="/dashboard.png" alt="logo" className='w-20 hidden md:flex' />
                <img src="/dashboard.png" alt="logo" className='w-8 flex md:hidden' />
            </div>

            <ul className='mt-6 space-y-6'>
                {
                    SIDEBAR_LINKS.map((link, index) => (
                        <li key={index} className={`font-medium rounded-md py-2 px-5 transition-all duration-200 ${activeLink === index? "bg-slate-800 text-indigo-400 shadow-inner": "text-gray-300 hover:bg-slate-800 hover:text-white"}`}>
                            <Link to={link.path} className='flex justify-center md:justify-start items-center md:space-x-5' onClick={() => handleLinkClick(index)}>
                                <span>{link.icon()}</span>
                                <span className='text-sm text-white hidden md:flex'>{link.name}</span>
                            </Link>
                        </li>
                    ))
                }
